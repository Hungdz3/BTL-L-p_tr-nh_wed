...
// Mặc định 0 và mảng rỗng nếu chưa có dữ liệu trong DB
if (empty($counts)) {
    $counts = ['sinh_vien' => 0, 'giao_vien' => 0, 'mon_hoc' => 0, 'lop_hoc_phan' => 0, 'thong_bao' => 0, 'tai_khoan' => 0];
}
if (!isset($new_students)) {
    $new_students = [];
}
if (!isset($new_teachers)) {
    $new_teachers = [];
}
