...
// Lấy danh sách tất cả Lớp học đính kèm sĩ số
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
    ...
    <!-- Card 2: Điểm TB Lớp -->
    <div class="bg-white p-3.5 rounded-xl border-2 border-indigo-900 shadow-sm flex justify-between items-start">
        <div>
            <p class="text-[10px] font-bold text-indigo-900 tracking-wider uppercase">ĐIỂM TB LỚP</p>
            <h3 class="text-2xl font-black text-indigo-900 mt-0.5"><?= number_format($stats['diem_tb'], 1) ?></h3>
            <p class="text-[10px] text-gray-500 mt-0.5">Hệ điểm 10</p>
        </div>
        <div class="p-2 bg-indigo-900 text-white rounded-lg shadow-sm">
                        ...
                        <tr class="hover:bg-indigo-50/40 transition group" data-mssv="<?= e($s['MSSV']) ?>">
                            ...
                            <td class="py-2 px-3 text-center">
                                <div class="flex items-center justify-center space-x-1.5">
                                    ...
                                    <!-- Nút XÓA -->
                                    <a href="actions/delete_student.php?mssv=<?= urlencode($s['MSSV']) ?>&ma_lop=<?= urlencode($maLop) ?>" 
                                       onclick="return confirm('Bạn có chắc chắn muốn XÓA học viên <?= e(addslashes($s['HoTen'])) ?> (MSSV: <?= e($s['MSSV']) ?>) khỏi lớp này không?')" 
                                       class="p-1.5 bg-rose-50 border border-rose-300 text-rose-700 hover:bg-rose-100 rounded shadow-sm transition" title="Xóa học viên">
...
<div id="addStudentModal" class="<?= $openModal ? '' : 'hidden' ?> fixed inset-0 z-50 overflow-y-auto bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-3">
    <div class="bg-white rounded-2xl max-w-md w-full p-5 shadow-2xl border border-gray-200 transform transition-all">
        ...
        <form method="POST" action="actions/add_student_grade.php" class="mt-3 space-y-3 text-xs" onsubmit="return validateMssvAddForm(this)">
            ...
            <div>
                <label class="block text-[11px] font-bold text-gray-700 mb-1">Mã số sinh viên (MSSV) <span class="text-red-500">* (Chỉ từ 1-10 chữ số)</span></label>
                <input type="text" name="mssv" required pattern="[0-9]{1,10}" maxlength="10" inputmode="numeric" value="<?= e($formData['mssv'] ?? '') ?>" 
                       placeholder="Ví dụ: 21120999" 
                       class="w-full px-3 py-1.5 border rounded-lg text-xs outline-none focus:ring-2 focus:ring-indigo-800 <?= isset($formErrors['mssv']) ? 'border-red-500 bg-red-50' : 'border-gray-300' ?>">
            ...
            <div>
                ...
                <input type="text" name="ho_ten" required value="<?= e($formData['ho_ten'] ?? '') ?>" 
                       placeholder="Ví dụ: Nguyễn Văn A" 
                       class="w-full px-3 py-1.5 border rounded-lg text-xs outline-none focus:ring-2 focus:ring-indigo-800 <?= isset($formErrors['ho_ten']) ? 'border-red-500 bg-red-50' : 'border-gray-300' ?>">
    ...
    <div class="bg-white rounded-2xl max-w-md w-full p-5 shadow-2xl border border-gray-200 transform transition-all">
        <div class="flex items-center justify-between border-b pb-2.5">
            <h3 class="text-sm font-bold text-amber-800 flex items-center gap-2">
                <i class="fa-solid fa-pen-to-square"></i>
                <span>Sửa Thông Tin & Điểm Học Viên</span>
            </h3>
        ...
        <form method="POST" action="actions/edit_student.php" class="mt-3 space-y-3 text-xs">
            ...
            <div>
                <label class="block text-[11px] font-bold text-gray-700 mb-1">Họ và tên học viên <span class="text-red-500">*</span></label>
                <input type="text" name="ho_ten" id="edit_ho_ten" required class="w-full px-3 py-1.5 border border-gray-300 rounded-lg text-xs outline-none focus:ring-2 focus:ring-amber-600">
            </div>
...
<script>
    ...
    function openEditModal(mssv, hoTen, cc, gk, ck) {
        ...
        document.getElementById('edit_diem_cc').value = parseFloat(cc).toFixed(1);
        document.getElementById('edit_diem_gk').value = parseFloat(gk).toFixed(1);
        document.getElementById('edit_diem_ck').value = parseFloat(ck).toFixed(1);
    ...
    function validateMssvAddForm(form) {
        ...
        if (!regex.test(mssv)) {
            alert("LỖI NHẬP LIỆU: Mã số sinh viên (MSSV) chỉ được nhập các chữ số (0-9) từ 1 đến 10 chữ số!");
            form.mssv.focus();
            return false;
        }
        return true;
