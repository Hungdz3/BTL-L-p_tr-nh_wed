...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'dang_ky' && $lhp_info) {
        ...
        $_SESSION['da_dk'] = array_values(array_filter(
            $_SESSION['da_dk'],
            function($item) use ($ma_lhp) {
                return $item['ma_lhp'] !== $ma_lhp;
            }
        ));
...
if ($keyword !== '') {
    $hien_thi = array_filter($hien_thi, function($lhp) use ($keyword) {
        return stripos($lhp['ten_mon'], $keyword) !== false ||
...
<!-- Footer -->
<footer>
  &copy; 2026 Trường Đại học HNDA. Tất cả các quyền được bảo lưu.
</footer>
